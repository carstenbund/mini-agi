...
DEFAULT_TEMPLATES: Dict[str, str] = {
    "bridge": (
        ...
        "Rules:\n"
        "- Your abstraction must be SPECIFIC to these two targets. If it "
        "would also bind two randomly chosen ideas (complexity, coherence, "
        "interconnectedness, balance, resonance, accountability, systems "
        "thinking), it is too broad — discard it and dig deeper.\n"
        "- Ground it: cite one concrete element FROM EACH target and show "
        "how your abstraction transforms it.\n"
        "- State one consequence NEITHER target already contains.\n\n"
        "Example of the required specificity (from another pair): CLAIM: "
        "measurement and belonging are one operation — a sensing loop with "
        "a return channel. FROM A: an indicator is society-scale "
        "co-regulation; legitimacy is the return channel. FROM B: belonging "
        "is two managers continuously indicating each other's states. NEW: "
        "surveillance is measurement minus reciprocity. PREDICTION: "
        "one-way measurement expansion raises anxiety at constant safety.\n"
        "Rejected example: 'the binding abstraction is Complexity' — "
        "rejected because it binds any two texts.\n\n"
        "Answer in exactly this structure:\n"
        "CLAIM: <the binding abstraction — a full sentence, never a one- or "
        "two-word category>\n"
        "FROM A: <how the first target instantiates it, citing its specifics>\n"
        "FROM B: <how the second target instantiates it, citing its specifics>\n"
        "NEW: <a consequence neither target states>\n"
        "PREDICTION: <one testable prediction>"
    ),
    "deepen": (
        ...
        "Rules: do not summarize or restate — every sentence must add a "
        "claim the motif does not contain. Name the mechanism, not a "
        "category. Answer in this structure:\n"
        "CLAIM: <the higher-level claim, a full sentence>\n"
        "GROUND: <which specifics of the motif it grows from>\n"
        "NEW: <what it adds that the motif lacks>\n"
        "PREDICTION: <one testable prediction>"
    ),
    "self": (
